...
public class AnimGLEventListener4 extends AnimListener implements MouseListener {
    ...
    public AnimGLEventListener4(int aiThinkDelay, boolean aiEnabled) {
...
//        this.currentLevelIndex=aiThinkDelay-2;
    ...
    private static final float AI_ATTACK_RANGE_SQUARED = 10000f; // 100²
    private static final float AI_CHASE_RANGE_SQUARED = 40000f; // 200²
    private int aiThinkDelay ; // Reduced from likely 20-30
    private float lastPlayerX, lastPlayerY;
    private float lastPlayer2X, lastPlayer2Y;
    private float predictedPlayerX = 0;
    private float predictedPlayerY = 0;
    private final float aiPredictionTime = 0.3f;
    ...
    private void updateAI() {
        ...
        // AI thinks much more frequently
        aiThinkTimer++;
        ...
        // Cache positions and only recalculate if needed
        float currentPlayerX = player.getX();
        float currentPlayerY = player.getY();
        float currentPlayer2X = player2.getX();
        float currentPlayer2Y = player2.getY();

        // Calculate delta to player with prediction
        float playerSpeedX = 0;
        float playerSpeedY = 0;

        // Simple velocity estimation (if you don't have velocity tracking)
        if (lastPlayerX != 0 && lastPlayerY != 0) {
            playerSpeedX = (currentPlayerX - lastPlayerX) * 10f; // Scale for prediction
            playerSpeedY = (currentPlayerY - lastPlayerY) * 10f;
        }

        // Predict player position
        predictedPlayerX = currentPlayerX + playerSpeedX * aiPredictionTime;
        predictedPlayerY = currentPlayerY + playerSpeedY * aiPredictionTime;

        // Calculate distance to predicted position (using squared distance for speed)
        float dx = predictedPlayerX - currentPlayer2X;
        float dy = predictedPlayerY - currentPlayer2Y;
        float distanceSquared = dx * dx + dy * dy;

        // Cache current positions
        lastPlayerX = currentPlayerX;
        lastPlayerY = currentPlayerY;
        lastPlayer2X = currentPlayer2X;
        lastPlayer2Y = currentPlayer2Y;

        // Faster, simpler decision making
        if (distanceSquared < AI_ATTACK_RANGE_SQUARED && !isJumping2) {
            // Always attack when in range and not jumping
            aiCurrentAction = "attack";
        } else if (player2Health < 30 && player1Health > player2Health + 15) {
            // Retreat when significantly disadvantaged
            aiCurrentAction = "retreat";
        } else {
            // Always chase otherwise (no idle time)
            aiCurrentAction = "chase";
        }

        // Execute AI action immediately
        executeAIAction(dx, dy, distanceSquared);
    }

    private void executeAIAction(float dx, float dy, float distanceSquared) {
        float moveSpeed = 10.0f;  // Increased from 8.0f
        float dashSpeed = 12.0f;  // For aggressive moves
        float chaseSpeed = 10.0f; // For chasing

        // Normalize direction for smooth movement (optional, can be removed if too heavy)
        ...
        switch (aiCurrentAction) {
            case "attack":
                // Aggressive attacking with approach
                ...
                // Dash toward player while attacking
                if (Math.abs(dx) > 30) {
                    float attackMoveSpeed = dashSpeed * 0.7f;
                    if (dx > 0) {
                        player2.move(attackMoveSpeed, 0);
                    } else {
                        player2.move(-attackMoveSpeed, 0);
                    }
                }
                ...
                // Jump attack combo
                if (Math.random() > 0.7 && !isJumping2 && !isFighting2) {
                    isJumping2 = true;
                    jumpFrameCounter2 = 0;
                    jumpVelocity2 = jumpStrength * 1.3f;
                    player2.startJumping();
                }
                break;

            case "chase":
                // Fast, direct chasing with normalized movement
                if (!isFighting2) {
                    // Move directly toward predicted position
                    if (Math.abs(dx) > 15) {  // Reduced threshold for immediate response
                        player2.move(dirX * chaseSpeed, 0);
                    }

                    if (Math.abs(dy) > 15) {
                        player2.move(0, dirY * chaseSpeed);
                    }

                    // More frequent jumping
                    if (Math.random() > 0.8 && !isJumping2) {  // Increased frequency
                        isJumping2 = true;
                        jumpFrameCounter2 = 0;
                        jumpVelocity2 = jumpStrength * 1.2f;
                        player2.startJumping();
                    }

                    // Attack while chasing if getting close
                    if (distanceSquared < AI_ATTACK_RANGE_SQUARED * 1.3f && !isJumping2) {
                        isFighting2 = true;
                        fightFrameCounter2 = 0;
                        player2.startFighting();
                    }
            ...
            case "retreat":
                // Fast retreat with dodging
                if (!isFighting2) {
                    // Move away from player
                    float retreatSpeed = moveSpeed * 1.1f;

                    if (dx > 0 && player2.getX() > player2.getWidth() / 2) {
                        player2.move(-retreatSpeed, 0);
                    } else if (dx < 0 && player2.getX() < maxWidth - player2.getWidth() / 2) {
                        player2.move(retreatSpeed, 0);
                    }

                    // Dodging jumps
                    if (Math.random() > 0.75 && !isJumping2) {
                        isJumping2 = true;
                        jumpFrameCounter2 = 0;
                        jumpVelocity2 = jumpStrength * 1.4f; // Higher jumps to escape
                        player2.startJumping();

                        // Add horizontal dodge during jump
                        if (dx > 0) {
                            player2.move(-retreatSpeed * 1.5f, 0);
                        } else {
                            player2.move(retreatSpeed * 1.5f, 0);
                        }
                    }
                    ...
                    if (distanceSquared < AI_ATTACK_RANGE_SQUARED * 0.8f &&
                            Math.random() > 0.9 && !isJumping2) {
                        isFighting2 = true;
                        fightFrameCounter2 = 0;
                        player2.startFighting();
                    }
        ...
        // Remove "idle" action completely - AI is always active

        // Additional: Always try to face the player
        if (dx > 0) {
